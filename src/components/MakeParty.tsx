...
export function MakeParty() {
    ...
    async function fetchItems() {
      ...
      try {
        ...
        const formattedItems: PartyItem[] = dataArray.map((item: ItemAPI) => {
          ...
          // 🚀 CORREÇÃO 1: Tenta PARSEAR strings malformadas como arrays (Ex: "[\"Pratos\"]")
          if (typeof tagsData === 'string' && (tagsData.startsWith('[') || tagsData.startsWith('"['))) {
            try {
                tagsData = JSON.parse(tagsData); 
            } catch (e) {
                console.warn("Erro ao parsear tag como JSON:", tagsData);
            }
          }

          // 1. Prioriza o campo 'categoria'
          if (item.categoria && typeof item.categoria === 'string' && item.categoria.trim() !== '') {
            rawCategory = item.categoria;
          } 
          // 2. Tenta usar a tag (que agora pode ser o array correto)
          else if (tagsData) {
            if (Array.isArray(tagsData) && tagsData.length > 0) {
              const firstTag = tagsData[0];
              if (firstTag && typeof firstTag === 'string') {
                  rawCategory = firstTag.trim();
              }
            } else if (typeof tagsData === 'string') {
              rawCategory = tagsData.split(',')[0].trim();
            }
          }
        ...
        console.error("ERRO AO BUSCAR ITENS na URL:", API_URL_FULL, error);
        toast.error("Erro fatal ao buscar itens. Verifique se o servidor está ativo.");
      } finally {
        setIsLoading(false);
      }
    }
    fetchItems();
  }, []); // 👈 CORREÇÃO 2: Array de dependências VAZIO para rodar apenas uma vez!

  // 2. Filtrar itens APENAS da etapa atual (continua o lowerCase para compatibilidade)
  ...
  const currentStepItems = useMemo(() => {
    ...
    return items.filter(item => 
      item.category.toLowerCase() === currentCategoryName.toLowerCase()
    );
  ...
  const handleNext = () => {
    ...
    if (currentStepIndex < ETAPAS_FESTA.length - 1) {
      ...
      <div className="container mx-auto px-4 py-16 min-h-[60vh] flex flex-col items-center justify-center text-center animate-in zoom-in duration-500">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
          ...
          >
            ...
            {/* O botão de Carrinho está no Header, mas você pode adicionar uma CTA aqui */}
    ...
    <div className="min-h-screen bg-gray-50/50 pb-24">
      ...
      <div className="sticky top-[64px] md:top-[80px] z-40 bg-white border-b border-gray-200 shadow-sm">
        <div className="container mx-auto px-4 py-4">
          ...
          {/* Barra Cinza e Preenchimento Rosa */}
          <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
            <div 
              className="bg-[#E91E63] h-full transition-all duration-500 ease-out"
        ...
        {currentStepItems.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 animate-in slide-in-from-right-8 fade-in duration-300">
            {currentStepItems.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md hover:border-pink-200 transition-all group flex flex-col"
              >
                <div className="relative aspect-square bg-gray-100">
                  <img
                    src={item.imagem_url || 'https://placehold.co/400x400?text=Item'}
                    alt={item.nome}
          ...
          /* Estado vazio da Categoria Atual */
          <div className="flex flex-col items-center justify-center py-12 text-center animate-in fade-in">
            ...
            <p className="text-gray-500 max-w-xs mx-auto mb-6">
              Não encontramos itens cadastrados nesta categoria específica.
            </p>
            <Button variant="ghost" onClick={handleNext} className="text-[#E91E63]">
              Pular para próxima categoria &rarr;
            </Button>
          </div>
        )}
      </div>

      {/* --- RODAPÉ DE NAVEGAÇÃO (STICKY BOTTOM) --- */}
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-50">
        <div className="container mx-auto max-w-4xl flex justify-between items-center gap-4">
          ...
          <Button
            ...
            className={`flex-1 md:flex-none ${currentStepIndex === 0 ? 'opacity-0 pointer-events-none' : ''}`}
          >
          ...
          <div className="hidden md:block text-sm text-gray-500">
            {currentStepItems.length} opções encontradas
          </div>

          <Button 
            size="lg"
            onClick={handleNext}
            className="flex-1 md:min-w-[200px] bg-[#E91E63] hover:bg-[#d81557] text-white font-bold rounded-xl shadow-lg shadow-pink-200 transition-all active:scale-95"
          >
            {currentStepIndex === ETAPAS_FESTA.length - 1 ? (
              ...
              <span className="flex items-center">
                Próximo Passo <ChevronRight className="w-5 h-5 ml-2" />
              </span>
            )}
          </Button>
        </div>
